...
namespace FFTColorCustomizer.Tests.Core
{
    public class UserConfigPathTests
    {
        ...
        {
            // TLDR: The mod MUST load the User config from Reloaded/User/Mods/prawl.fft.colorcustomizer/Config.json
            ...
            var userConfigPath = Path.Combine(reloadedRoot, "User", "Mods", ColorModConstants.ModNamespace, "Config.json");
            var modConfigPath = Path.Combine(modPath, "Config.json");

            // The correct path should be the User config
            userConfigPath.Should().Contain($@"User\Mods\{ColorModConstants.ModNamespace}",
        ...
        public void GetUserConfigPath_Should_Return_User_Directory_When_It_Exists()
        {
            ...
            var userModsDir = Path.Combine(testRoot, "User", "Mods", ColorModConstants.ModNamespace);
        ...
        public void ConfigurationCoordinator_Should_Pass_Correct_ModPath_For_Previews()
        {
            ...
            var userConfigPath = $@"C:\Reloaded\User\Mods\{ColorModConstants.ModNamespace}\Config.json";

            // The mod path passed to ConfigurationForm should be the parent of Config directory
            var expectedModPath = $@"C:\Reloaded\User\Mods\{ColorModConstants.ModNamespace}";
            ...
            previewPath.Should().Be($@"C:\Reloaded\User\Mods\{ColorModConstants.ModNamespace}\Resources\Previews",
