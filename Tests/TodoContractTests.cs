...
namespace FFTColorCustomizer.Tests
{
    ...
    /// FFTLivingWeapons TodoContractTests pattern with the CC-n id prefix. RepoRoot walks up
    /// from the test bin dir to the first dir holding both docs/TODO.md and ColorMod/.
    ///
    ...
    /// F. Every CC-id captured out of TODO.md's entries and CHANGELOG.md's entries is unique across
    ...
    public class TodoContractTests
    {
        private static string RepoRoot()
        {
            ...
            throw new FileNotFoundException("repo root (docs/TODO.md + ColorMod/) not found above the test bin dir");
        }
        ...
        private static readonly Regex NowEntryRegex = new(
            @"^- \*\*\[CC-(\d+)\] [^*]+\*\* \(opened \d{4}-\d{2}-\d{2}\) \[(QUEUED|BUILDING|AWAITING-LIVE|BLOCKED\([^)]+\))\]$",
            RegexOptions.Compiled);

        private static readonly Regex BacklogEntryRegex = new(
            @"^- \[CC-(\d+)\] \d{4}-\d{2}-\d{2}: .+", RegexOptions.Compiled);

        private static readonly Regex ChangelogEntryRegex = new(
            @"^- \[CC-(\d+)\] (SHIPPED [0-9a-f]{7,40} |WONTFIX |RETRACTED )\d{4}-\d{2}-\d{2}: .+",
            RegexOptions.Compiled);
        ...
        /// opening bracket (say "- CC-9 SHIPPED abc1234 2026-07-21: text") dodge both the grammar
        ...
        [Theory]
        [InlineData("- **[CC-1] Some title here** (opened 2026-07-21) [QUEUED]", true)]
        [InlineData("- **[CC-2] Some title here** (opened 2026-07-21) [BUILDING]", true)]
        [InlineData("- **[CC-3] Some title here** (opened 2026-07-21) [AWAITING-LIVE]", true)]
        [InlineData("- **[CC-4] Some title here** (opened 2026-07-21) [BLOCKED(owner live session)]", true)]
        [InlineData("- **[CC-5] Some title here** (opened 2026-07-21)", false)]                          // missing status token
        [InlineData("- **[CC-6] Some title here** (opened 2026-07-21) [WAITING]", false)]                 // unknown status token
        [InlineData("- **[CC-7] Some title here** (opened 2026-07-21) [BLOCKED]", false)]                 // BLOCKED needs a reason
        [InlineData("- **[CC-8] Some title here** (opened 2026-7-21) [QUEUED]", false)]                   // bad date shape
        [InlineData("- [CC-9] Some title here (opened 2026-07-21) [QUEUED]", false)]                      // missing bold markers
        [InlineData("- **[CC-10] Title** rogue **[CC-11] second marker** (opened 2026-07-21) [QUEUED]", false)]  // greedy title must not swallow a second "**[" marker
        ...
        [Theory]
        [InlineData("- [CC-10] 2026-07-21: One sentence description.", true)]
        [InlineData("- [CC-11] 2026-07-21: ", false)]                          // no sentence text after the colon
        [InlineData("- [CC-12] 2026-7-21: One sentence description.", false)]   // bad date shape
        [InlineData("- **[CC-13] Some title** 2026-07-21: text", false)]       // bold markers not allowed here
        [InlineData("- CC-14 2026-07-21: text", false)]                        // missing the [CC-n] brackets
        ...
        [Theory]
        [InlineData("- [CC-15] SHIPPED 58d5c7b 2026-07-21: summary text.", true)]
        [InlineData("- [CC-16] WONTFIX 2026-07-21: summary text.", true)]
        [InlineData("- [CC-17] RETRACTED 2026-07-21: summary text.", true)]
        [InlineData("- [CC-18] SHIPPED 2026-07-21: summary text.", false)]           // SHIPPED without a hash
        [InlineData("- [CC-19] SHIPPED zzzzzzz 2026-07-21: summary text.", false)]   // hash not hex
        [InlineData("- [CC-20] DEFERRED 2026-07-21: summary text.", false)]         // unknown disposition
        [InlineData("- [CC-21] SHIPPED 58d5c7b 2026-7-21: summary text.", false)]    // bad date shape
        [InlineData("- CC-22 SHIPPED abc1234 2026-07-21: summary text.", false)]     // missing the [CC-n] brackets
        ...
        public void ChangelogEntryLines_catches_a_bracketless_entry_that_a_bracket_prefix_filter_would_miss()
        {
            ...
            var lines = new[]
            {
                ...
                "## 3.2.0 cycle",
                "",
                "- [CC-1] SHIPPED abc1234 2026-07-21: a well-formed entry.",
                "  continuation prose for CC-1.",
                "- CC-9 SHIPPED abc1234 2026-07-21: mangled entry missing its opening bracket.",
            };

            var scanned = ChangelogEntryLines(lines);

            Assert.Contains(scanned, l => l.Contains("CC-9"));
            string mangled = scanned.Single(l => l.Contains("CC-9"));
        ...
        [Fact]
        public void Every_CC_id_is_unique_across_TODO_and_CHANGELOG()
        {
            string repoRoot = RepoRoot();
            ...
            Assert.True(duplicates.Count == 0,
                "Duplicate CC-ids found:\n" + string.Join("\n",
                    duplicates.Select(g => $"CC-{g.Key}: " + string.Join(" | ", g.Select(x => x.Source)))));
        }
