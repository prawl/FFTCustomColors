...
namespace FFTColorMod.Tests
{
    public class SimplePropertyTest
    {
        ...
        public void TestPropertyLookup()
        {
            ...
            Assert.Equal(typeof(FFTColorMod.Configuration.ColorScheme), archerFemaleUnderscore.PropertyType);

            // Create a config and test setting the value
            var config = new FFTColorMod.Configuration.Config();

            // Set using reflection
            var lucaviEnum = FFTColorMod.Configuration.ColorScheme.lucavi;
            ...
            // Check if the SetColorSchemeForJob can find the property
            ...
            Assert.NotNull(prop);
            Assert.Equal(typeof(FFTColorMod.Configuration.ColorScheme), prop.PropertyType);
        }
    }
}