...
2. Every file matched by `FFTHandsFree/Instructions/*.md` — at minimum:
   ...
   - `BattleTurns.md` — turn flow, scan output (NOTE: read the
     "Multi-unit party turn-cycling" + "Per-tile annotation tags" sections)
...
mid-fight. **Lloyd (Orator) is the active player unit**. He may be the
last man standing — other allies are likely KO'd from prior runs. 3
live enemies on the field. Your first action is `screen` to see what
you're working with. The game is on Windows, running via Reloaded-II.
The bridge is single-threaded — wait for each command's response before
sending the next.
...
### `playtest_logs/2026-04-26_0157/play.md`
...
### `playtest_logs/2026-04-26_0157/feedback.md`
...
- **Don't ask the parent for guidance** — operate solo. If genuinely
  stuck, log the situation and recover (e.g. `battle_flee` or the new
  `return_to_my_turn` helper for in-battle sub-menu escape).
