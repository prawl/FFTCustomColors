...
#   +0x12 Accessory = 239 (Sortilege, perfume)
#   +0x14 Weapon    = 37  (Chaos Blade, WP 40, range 1, evade 20)
#   +0x1A Shield    = 143 (Escutcheon (strong), PhysEv 75, MagEv 50)
...
god_ramza() {
  ...
  _write_pair 0x12 239   # Sortilege (perfume)
  _write_pair 0x14 37    # Chaos Blade
  _write_pair 0x16 255   # Clear left-hand
  _write_pair 0x18 255   # Clear reserved
  _write_pair 0x1A 143   # Escutcheon (strong)
...
// can't be attacked, so listing them as targets was misleading.
...
// Up to 5 entries to keep the line short.
...
us.forEach(u=>{
  ...
  // playtest: undisclosed Wisenkin Counter killed Ramza when these were
  // verbose-only.
