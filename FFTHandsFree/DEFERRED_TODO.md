...
- [ ] **Mid-scene speaker accuracy in .mes-derived dialogue** — `MesDecoder.DecodeBoxes` currently emits `null` (→ "narrator" in fft.sh) for any segment that doesn't carry its own `0xE3 0x08` marker. The .mes file only encodes speaker at scene transitions; mid-scene speaker changes come from the .evt event script which we don't parse. Three options ranked by effort: (a) hand-curate per-event speaker arrays keyed by box index — fragile to pagination drift, manual per event, (b) memory hunt for the runtime speaker portrait/name during BattleDialogue — most robust, needs a test cycle with snapshot/diff, (c) parse `event_test_evt.en.bin` — full solution, substantial RE work. Surfaced live 2026-04-26 at event 045: bridge says "narrator" for boxes 4–11 but game shows Dycedarg / Ramza / Delita rotating. See `project_dialogue_decoder_2026_04_26.md`.
