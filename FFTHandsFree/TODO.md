...
- [x] **Enemy reaction abilities** [EnemyIntel] — Read equipped reaction/support/movement abilities from heap struct bitfields at +0x74 (reaction, 4b, base 166) and +0x78 (support, 5b, base 198). MSB-first bitfields decoded by `PassiveAbilityDecoder`. Player units read from roster (+0x08/+0x0A/+0x0C). Shell shows `equip: R:Parry | S:Equip Swords`. Verified: Knight(Parry ✓, Equip Swords ✓), Archer(Gil Snapper ✓, Evasive Stance ✓). 2026-04-12.
...
- [x] **Flesh out validPaths for every screen state** — Added paths for Battle_Formation (place units, commence), Battle_Abilities (submenu navigation), and all Battle_<Skillset> screens (ability list browse/select/cancel). All named screen states now return validPaths. Remaining: Settlement/Shop screens (not yet detectable). 2026-04-12.
- [x] **Skip C+Up scan when consecutive friendly turns** — SUPERSEDED: C+Up scanning eliminated entirely. All unit data now read from static battle array at 0x140893C00 (stride 0x200). Grid positions at +0x33/+0x34, HP/stats/status all from memory. Zero keyboard input, ~15ms per scan. 2026-04-12.
...
- [ ] **`screen` shows wrong active unit between scans** [State] — `screen` reads the active unit from a stale memory buffer that doesn't update reliably between turns. Observed: showed "Wilham(Summoner)" when the actual active unit was Kenrick(Archer). Fix: cache the active unit identity from `scan_move` and use it in `screen` output. If no cached scan exists (or cache is stale), omit the active unit from `screen` rather than showing a wrong one. Observed 2026-04-12.
...
- [ ] **battle_attack allows diagonal targets** [Execution] — `battle_attack x y` doesn't validate that the target is on a cardinal direction (same X or same Y) from the attacker. FFT only allows attacking in the 4 cardinal directions. Claude tried to attack (7,11) from (8,10) which is diagonal — the attack animation played but targeting was wrong. Fix: validate `targetX == unitX || targetY == unitY` before entering targeting mode, return an error explaining cardinal-only constraint. Observed 2026-04-12.
- [ ] **Verify attack landed** [Execution] — Check enemy HP decreased after attack animation. Currently reports false "MISSED" when the attack landed but the condensed struct hadn't updated yet (observed with gun attack on Gobbledygook — hit confirmed visually but HP check read stale data). Low priority: Claude already sees updated HP on next scan_move. Consider removing the MISS detection entirely since it's unreliable and misleading. Observed 2026-04-12.
...
- [x] **Scan misidentifies active unit after ability-only turn** [Movement] — Could not reproduce. Kenrick used Chakra (self-radius, ability-only), next scan correctly identified Ramza. Original observation may have been a one-off timing issue with Cutscene event (401). Closed 2026-04-12.
- [ ] **C+Up scan sometimes fails to detect units after restart** [Movement] — After `restart`, the C+Up cycle only picks up 1 unit (the active unit) instead of all battlefield units. Subsequent scans also fail. The camera may be in a bad state or the C+Up key isn't reaching the game. Manual play works fine. May need to reset camera position or press a key to re-initialize the scan cycle. Observed 2026-04-12.
- [ ] **Jump ends turn immediately — no Wait/facing step** [Execution] — After `battle_ability "Jump"`, the unit leaps into the air and the turn ends immediately. There is no Wait menu or facing confirmation. Currently Claude will try to `battle_wait` after Jump which will fail or act on the wrong unit. Need to detect Jump as a turn-ending ability and skip Wait. May also apply to other charge-time abilities. Observed 2026-04-12.
...
- [~] **Auto-detect battle map** [Movement] — Location ID lookup + random encounter maps implemented, fingerprint fallback. BUG: after restart, location address (0x14077D208) reads 255 and last_location.txt may be missing, causing wrong map auto-detection (e.g. MAP085 Mandalia Plain instead of MAP074 Siedge Weald). Need to persist location more reliably or detect from encounter data. Observed 2026-04-12.
...
- [x] **BattleBasics.md** — Covered by `BattleTurns.md`: turn flow, scan_move output, battle_ability usage with all targeting types (point, AoE, line, self, self-radius, full-field), quick reference examples. Updated 2026-04-12.
