...
- [ ] **`world_travel_to` accepts a location name, not just ID** — Currently the shell wrapper requires a numeric ID (`world_travel_to 11` for Goug). Should accept the display name (`world_travel_to "Clockwork City of Goug"` or fuzzy `"Goug"`) and resolve to ID via the existing location-name table (see `project_location_ids_verified.md` — all 43 IDs confirmed). Implement in `fft.sh` helper: if arg is non-numeric, lookup via a name→ID map (could auto-derive from a `screen` query on the location's tooltip or hardcode from the verified list). Requested mid-session 2026-04-19 while hunting a Poachers' Den — I picked wrong city by ID because name lookup was manual.
