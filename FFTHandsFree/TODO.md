...
- [~] **Live-verify `execute_action` responses include `ui=` field across battle screens — MOSTLY DONE (session 44 parts 1+5)** — Live-tested in a Mount Bervenia random encounter. Findings: **ui POPULATES** on `BattleMyTurn` ("Move"/"Abilities"/"Status"), `BattleMoving` (tile "(8,7)"), `BattleAbilities` ("Attack"), `BattleAttacking` ("Attack" via tracker, or "(x,y)" via new cursor fallback). **`BattleStatus` fix shipped + root-cause bug found + fixed**: A hidden "EqA-promote" block at CommandWatcher:6190 was unconditionally renaming `screen.Name` from `BattleStatus` → `EquipmentAndAbilities` whenever the mirror matched the active unit's equipment — stripping the battle context. Fix: exclude `BattleStatus` from promotion. NEW pure-class `BattleStatusUiResolver` + 2 tests; `screen.UI` set from `_cachedActiveUnitName`. Post-fix log confirmed `ui='Kenrick'` set correctly inside CommandWatcher. Subtle second-order issue: between `execute_action Status` and the subsequent `screen` call, `_cachedActiveUnitName` appeared to revert to null in one observed instance — may be a race / extra DetectScreen call during state-machine resync. Did NOT resolve in-session; compact shell rendering shows "Kenrick" via ASUM field so user-visible behavior improved even with the race. **Remaining for next session**: root-cause why `_cachedActiveUnitName` goes null between two consecutive screen reads; and BattlePaused ui decode (cursor byte CANDIDATES found — see next line). **BattleActing / BattleCasting / BattleWaiting** not exercised this session — TargetingLabelResolver change covers BattleCasting structurally.

- [ ] **BattlePaused cursor byte — hunt DONE, wiring PENDING (session 44 pt 5)** — Triple-diff identified 3 heap addresses (`0x132AAE6A8`, `0x132AAE6D8`, `0x132AAE6F0`) that track the pause-menu cursor row in lockstep. Verified live across rows 0, 1, 3. Addresses shuffle across restarts — per memory note `project_battle_pause_cursor.md`, the triple-diff technique from TavernRumors hunt (session 44 pt 2) is the reliable re-locator. **Remaining**: wire a `ResolveBattlePauseCursor` resolver in CommandWatcher (same shape as JobCursor/PickerCursor resolvers), map the 6-item menu (Data/Retry/Load/Settings/ReturnToWorldMap/ReturnToTitle) to strings, set `screen.UI` on BattlePaused. Non-blocking.
...
- [~] **Warriors' Guild: add `WarriorsGuild` screen state — PARTIAL (verified session 44 2026-04-18 at Bervenia)** — Screen state `WarriorsGuild` is already detected (not SettlementMenu) — name registers correctly. `shopSubMenuIndex = 20` at Bervenia guild. **Live-finding**: Bervenia guild only shows ONE sub-action visible, "Recruit" (not Recruit+Rename+Dismiss as TODO assumed). CursorDown/Up don't change `menuCursor` — either there's only one item, or the pause-menu-style custom cursor byte applies here too (same gap as BattlePaused — session 44). Screenshot confirmed single "Recruit" button. **Remaining**: (a) named `Recruit` validPath wrapping Enter; (b) confirm at other cities — Rename/Dismiss may be elsewhere in the UI tree (Party Menu? context-gated?); (c) find the guild's per-item cursor byte if multi-item list exists at other settlements.
