...
- [ ] **🔴 BattleDialogue misdetected as TravelList** [Detection] — Live-repro 2026-04-24: user loaded a saved game at Zeklaus Desert with a BattleDialogue (pre-battle story scene) active, but `screen` returned `[TravelList] curLoc=Zeklaus Desert` consistently across multiple polls. rawLocation=28 (Zeklaus) + stale party/ui flags from pre-load likely match the TravelList rule at `ScreenDetectionLogic.cs:471-472` (`party==0 && ui==1`) before the BattleDialogue rule gets a chance to fire. Root cause audit: dialogue detection requires `IsMidBattleEvent(eventId)` or similar — if eventId is in the pre-battle cutscene range (1..399) but the battle sentinels (slot0=255, slot9=0xFFFFFFFF) aren't set yet, the Cutscene/BattleDialogue path won't fire and we fall through to world-side rules. Fix path: either loosen the BattleDialogue rule to accept pre-battle event states, or tighten the TravelList/WorldMap rules to check eventId first (if an event is active, we're NOT on a plain world-map screen).
...
- [ ] **🔴 PLANNING-HEAVY: menuCursor byte keeps drifting from visible cursor state** [Detection] — Recurring bug across sessions: `ui=X` on BattleMyTurn reads a stale menuCursor (0x1407FC620) after non-action transitions (pause-menu escape, mid-submenu escape, state restoration). Live-repro 2026-04-24 Siedge Weald: cursor visually on Wait, scan reported `ui=Abilities`. The existing `_actedThisTurn` / `_movedThisTurn` correction only handles post-action stales. The NOT-acted-NOT-moved-but-navigated case is currently uncovered. **Don't tackle this without a planning pass first.** Options are NOT mutually exclusive, all have tradeoffs: (a) reset menuCursor byte to 0 (Move) on fresh-BattleMyTurn detection — needs reliable "fresh entry" signal across paths (enemy turn end, pause escape, etc); (b) shadow the cursor via key-press history in CommandWatcher — robust but requires tracking every key path; (c) WRITE memory after known navigations to force the byte into sync — leaks mod state into game memory, feels invasive; (d) drop the ui= label entirely on transitions we can't verify and render `ui=?` — safe but loses info. Estimate 2-3 hours to spec, another 2-3 to implement + TDD. Read `project_sm_cursor_tracking_pattern.md` first — it's the pattern that worked for non-battle cursors.

- [ ] **🟡 Player facing byte reads wrong value — off from visible direction** [Scan] — Live-repro 2026-04-24 Lenalian Plateau: Ramza visually facing West (same as adjacent Archer), memory decoder returned `facing=East` for Ramza but all 6 enemies' facing bytes matched their visual directions correctly. Enemy facings come from the static battle array slot +0x35 (FacingByteDecoder: 0=S, 1=W, 2=N, 3=E). Player slot may use a different layout / address, OR the player's facing byte doesn't update after movement completes, OR Ramza's slot is being resolved to the wrong index. Next step: live-inspect Ramza's slot bytes at slot_base+0x30..+0x36 after setting known facings via `battle_wait` N/S/E/W and recording memory bytes. If the byte IS updating correctly but we read the wrong one, the resolution is an index fix. If the byte ISN'T updating, find where the game writes player facing (probably a different address).

- [ ] **🟡 `Recommend Wait: Face X` picks wrong direction + arc counts off** [Scan] — Live-repro 2026-04-24 Lenalian Plateau: Ramza at (8,2) with enemies in every cardinal direction. Scan showed `Recommend Wait: Face North — 2 front, 4 side, 0 back` but manual count of adjacent threats put West as a better choice. Also the arc distribution (2/4/0 front/side/back for North) didn't match a hand-computation (0/4/2 expected by dot-product analysis). Likely a coordinate-system inconsistency between FacingStrategy's `CardinalFacings` (North = 0,+1) and grid coords (y+ = South). Audit FacingStrategy.ComputeOptimalFacingDetailed: flip y-axis for "North" if grid uses screen-coord convention, add characterization tests pinning expected arc counts for known enemy configurations. — Live-repro 2026-04-24 Lenalian Plateau: Ramza at (8,2) actually facing West with arcs `2 front, 1 side, 1 back`; scan reported `Face North — 2 front, 4 side, 0 back`. Both the direction label AND the arc distribution are off — the FacingStrategy uses `_lastDetectedRightDelta` empirically detected during grid nav, and if the camera rotated since the last move, that delta is stale. Fix path: either re-detect rotation on every scan (fire a harmless arrow key + read cursor delta), or read the camera rotation byte at 0x14077C970 directly (already have the address from project_battle_coords.md) and derive Right-delta from it.
