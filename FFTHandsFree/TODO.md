...
- [~] **🟡 `Recommend Wait: Face X` picks wrong direction + arc counts off** [Scan] — PARTIALLY SHIPPED `c36ec53` 2026-04-24: FacingDecider N/S label flip (+y is south in FFT grid). Label direction now matches visual direction. **Arc-count computation was NOT changed** — `FacingStrategy.ComputeOptimalFacingDetailed` still uses its internal `CardinalFacings` table, which may or may not produce accurate front/side/back counts. Live-repro 2026-04-24 showed 2/4/0 front/side/back for a Ramza+6-enemy setup where hand-computation said 0/4/2 was expected. Audit needed: trace which (dx,dy) vectors land in which arc for known enemy configurations; add characterization tests pinning expected arc counts.
