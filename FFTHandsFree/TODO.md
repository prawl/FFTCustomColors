...
- [x] **EqA row resolver: re-fire on detect-drift events** — shipped session 29 pt.13. `_lastEqaMenuDepth` tracks the previous DetectScreen's MenuDepth while on EqA. When MenuDepth JUST transitioned from >2 (inside a picker / CharacterStatus side column) back to 2 (EqA main), the resolver re-fires — costs ~2s per picker close but prevents stale `ui=Right Hand (none)` after equipment edits. Both latches clear on EqA exit.
...
- [x] **Apply mirror technique to JobSelection** — done. `CommandWatcher.cs:6335-6361` reads `_resolvedJobCursorAddr` → flat idx → row/col via `JobGridLayout.IndexToRowCol`, then calls `ScreenMachine.SetJobCursor(row, col)` when SM and memory disagree. `SetJobCursor` has bounds-checks. Blocked only by JobCursor resolver failing to find a live byte on this save (see separate TODO entry) — when a live byte is found the drift correction kicks in automatically. Session 29 pt.13 verified code path already in place.
