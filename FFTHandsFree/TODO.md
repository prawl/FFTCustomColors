...
- [ ] **Decode row index to item name** — with the cursor index known, resolve `ui=<item name>` by looking up the row's position in the shop's stock list. Partial data found:
  - Master weapon-tab item name list at `0x6007BE4` — UTF-16LE strings in a flat array (`Oak Staff\x00\x17White Staff\x00\x1BHealing Staff\x00...`). Contains ALL weapons in the game, not just this shop's stock.
  - Length/delimiter byte between entries (0x13, 0x17, 0x1B, etc. — appears proportional to NEXT string length).
  - Per-category: each tab (Weapons/Helms/Armor/etc.) has its own master list nearby.
  - **Missing:** which subset of the master list this specific shop sells, and at what prices. The shop probably has a small array of `(masterIndex, price, stockRemaining?)` records keyed by shop ID. Scan methodology: at Dorter Outfitter compare live memory to a SECOND outfitter with different stock (Warjilis or Gariland). Addresses that change between shops are the stock-selector data.
