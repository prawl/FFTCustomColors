...
(All shipped items archived to [COMPLETED_TODO.md](COMPLETED_TODO.md) under the 2026-04-25 / 2026-04-26 / 2026-04-28 entries.)
...
(Narrator damage/KO gap shipped 2026-04-26 — `removed`-with-`OldHp>0` now emits a death line; `PhantomKoCoalescer` suppresses the false-positive shape from transient bad scans.)

(Phantom cascade root fix shipped 2026-04-26 commit `67085f1` — HP-match active-dedup now resets Team=1 on demoted phantoms. Iter4 verify confirmed P1=0 / `allies=1` matches reality / Victory not Desertion. Follow-ups in `af8cf6f`, `0f9f7aa`, `9e2ab96` extended the fix; some residual phantom narrator events still appear — see open item below.)

### Open from this session

- [ ] **Residual phantom narrator burst (~5-14 events per `battle_wait`)** [Bridge/Narrator] — Iter5 + hand-play verify reduced from ~25 to ~5-14 events/turn but did NOT eliminate the cascade. Even with phantoms dropped from `units` list (`0f9f7aa`) and full identity reset (`af8cf6f`) and origSlotPos restore (`9e2ab96`), narrator still emits "Ramza took N damage", "Ramza died", "Ramza recovered N HP" while real Ramza is at full HP. **Stashed: a partial fix** that gates the `ReadGridPos()` cursor override on screen state (BattleMyTurn / BattleMoving / BattleAttacking / BattleAbilities only — skip during BattleEnemiesTurn / BattleAlliesTurn). It helped but didn't finish the job. Hypothesis for next angle: between turns, the condensed struct's `activeHp` lags. RosterMatcher Pass 1 may match a transient enemy unit (whose temp Brave/Faith reads coincidentally align) to Ramza's roster slot, briefly giving them his name. Would need Pass 1 to also require Team consistency to prevent the brief mis-attribution. To re-attempt: `git stash pop` the stashed change OR redesign Pass 1's match check. Cross-ref `project_phantom_cascade_root_2026_04_26.md`.

- [ ] ⚠ UNVERIFIED — **Alive-prefer attack-tile render** [Bridge/Render] — Shipped 2026-04-28 in `9e2ab96` (NavigationActions attack-tile renderer prefers `Hp > 0` occupant). Not exercised live this session — no enemy walked onto a corpse tile during the verify run. Confirm next session if a battle produces the shared-tile (corpse + alive on same tile) scenario; bridge should now show the alive enemy in `Right→(X,Y) enemy <Name>` instead of `treasure`. Pre-fix repro: any battle where an enemy crystallizes and another walks onto that tile.

- [ ] **`Phlegrai` goblin / multi-battle progression flow** [Bridge/State] — During the verify run, after winning the first Brigands' Den battle (Victory + crystal-restore prompt), the bridge auto-progressed into a SECOND battle (Goblin "Phlegrai" Lv12) without a clear transition. Unclear if this was an `execute_turn` background-job that kept driving past the Victory, or a legitimate next encounter. Reproducible? Capture the screen sequence from final-attack-Victory through to next BattleMyTurn and verify the bridge handles it cleanly (or splits the flow into discrete commands).
...
- [ ] **AttackOutcomeClassifier reports MISS when target dies AFTER our postHp read** [Bridge/Outcome] — Live-flagged 2026-04-26 playtest at Siedge Weald: Ramza basic-attacked Black Goblin at (3,4); bridge reported `MISSED!`; the Goblin was actually KO'd a beat later (visible as `HP=0/57 DEAD` in the next scan). The `BattleAttacking + postHp=0 → KO` cross-check shipped earlier this session catches the postHp=0 case, but here postHp was probably read while the Goblin was still alive (HP=57) — the Counter or status-add finished it AFTER our read. Fix path: settle longer before the postHp read on basic Attack, OR re-read postHp once after a 250ms delay if the screen is BattleMyTurn (i.e. the action resolved cleanly), OR retroactively reclassify if the next narrator scan shows the target at HP=0.

- [ ] **Goblin death not in narrator diff** [Bridge/Narrator] — Live-flagged 2026-04-26 playtest at Siedge Weald: Black Goblin went HP=57 → HP=0 between two scans but no `> Black Goblin died` (or `damaged`) line appeared in the narrator output. The `damaged HP→0` and `ko` event paths in `BattleNarratorRenderer` exist; the gap is upstream — likely the wait-poll's pre-snap caught the Goblin already at HP=0 (so no transition to surface), because the post-attack KO settled before the wait's first snapshot. Fix path: the active player's most-recent post-action snap should seed the next wait-poll's pre-snap so HP transitions span the action gap.
...
- [ ] **Guest allies mis-classified as ENEMY at Brigands' Den** [Bridge/Team] — Live-flagged 2026-04-26 playtest at Brigands' Den (event 048-ish, post-Eagrose Argath-joins). Both Argath at (4,1) lv=4 HP=61 AND Delita at (3,5) lv=3 HP=26/56 surface as `[ENEMY]` in scan_move output, when both are actually ALLY guests fighting alongside Ramza vs Milleuda's Corpse Brigade. Critical for tactical play — an agent trusting the bridge would attack its own teammates. The team byte read may be polluted by stale data from the prior battle / cutscene transition. Repro: enter Brigands' Den story battle (post-Eagrose, Argath joins), `scan_move`, observe the 8-unit roster — 2 of them are mis-tagged. Cross-ref `feedback_cache_preserve_on_null_active_unit.md` (similar caching shape but for active unit).

- [ ] **Delita mis-fingerprinted as "Skeleton" at Brigands' Den** [Bridge/Fingerprint] — Same playtest as above: Delita (lv=3 HP=26/56) surfaces as job=Skeleton in scan output. Probably a class-fingerprint collision — Delita's stat fingerprint at this level happens to match a Skeleton's. Fix path: layer the roster nameId ahead of class fingerprint when both available, OR exclude story-character-specific fingerprints from the generic-class lookup. Cross-ref `project_scan_diff_identity_collision.md` and `ClassFingerprintLookup.cs`.

(Multi-sample debounce + transition state machine SHIPPED 2026-04-26 in commits `4b4b65c` `ScreenTransitionValidator` and `b90cae0` `MajorityVote` 3-sample debounce in `DetectScreenSettled`. See `COMPLETED_TODO.md` 2026-04-28 entry.)

(Post-victory transient BattleDesertion mis-tag — partially addressed via the terminal flicker rules in `ae2968c` ScreenTransitionValidator allowing Desertion↔Victory swaps. Hand-play verify saw a clean Victory transition without lock-in. Re-flag if a future playtest sees Desertion lock-in despite the swap rule.)
