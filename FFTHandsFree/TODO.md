...
- [ ] **Damage detection in battle_attack response unreliable** [State] — BattleTracker detects damage events (visible in logs) but `GetRecentEvents` in battle_attack misses them. Root cause: enemy slot addresses in static array may not match the polling slots, or damage events are captured after the response is already built. The 63-damage-to-Lloyd was detected but 378-damage-to-enemy was not. Needs investigation into which array slots map to which units. Observed 2026-04-12.
- [ ] **Attack target tiles include diagonal targets** [Abilities] — Lloyd(Dragoon) at (10,10) shows Ramza at (9,9) as an Attack target, but FFT melee attacks (including spears) only work in cardinal directions (same X or same Y). `GetValidTargetTiles` needs to filter to cardinal-only tiles for melee/spear Attack. Currently uses diamond distance which includes diagonals. Observed 2026-04-12.
