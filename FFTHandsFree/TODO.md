...
- [ ] **Active unit position goes stale after a move — 2026-04-19 session 48** — Confirmed live at Zeklaus: Ramza starts at (4,0), moves to (6,1) via Enter+Up+Up+Left+F (game shows him at new tile, confirmed visually). Subsequent `scan_move` still reports Ramza at (4,0). The battle-array slot gridX/gridY bytes at +0x33/+0x34 either lag OR we're reading the wrong slot after the move. Blocks LoS live-verification (Attack tiles compute from stale position so the 15-tile range doesn't reach enemies that the in-game UI says are reachable). Fix path: (a) post-move, re-locate Ramza's slot by fingerprint (HP/MaxHP match) rather than caching the slot index, OR (b) find a true-live position address (heap unit struct x/y). Related: `feedback_partymenu_roster_sync.md` pattern — writes/reads to slots can drift when game internals re-sync from a master.
