...
- [ ] **`battle_wait` over-presses Down when DontAct grays out Act / cursor auto-lands on Wait** [Bridge/Nav] — When the active unit has DontAct (or any can't-act status), the in-game menu cursor auto-skips Move→Act and lands on Wait by default. `battle_wait` then presses Down once unconditionally to "navigate to Wait" — but the cursor was already there, so Down moves to Status (next slot). Live-flagged 2026-04-26 playtest #9: Wilham DontAct'd, executed `battle_wait` after move, ended up cycling through Status / AutoBattle. Fix: detect the menuCursor position before pressing Down — if already on Wait (slot 2), commit Enter directly. Or: read disable-status before nav and set the expected starting slot accordingly. Reading menuCursor is at `0x1407FC620` per [CommandWatcher.cs:5481](ColorMod/Utilities/CommandWatcher.cs#L5481).
