...
- [ ] **Live-verify MovementBfs edge-height fix against user visual count** — Session 29 pt.5/6 ported `TileEdgeHeight` (canonical FFT slope rules) into `MovementBfs.cs`. The BFS now compares exit-edge of tile A vs entry-edge of tile B instead of averaged display heights — this matters on slope-heavy maps (`Incline N/S/E/W`, `Convex/Concave *`). Old 13 BFS tests still green, 2 new integration tests guard the slope behaviour. **Not live-verified against the known-bad Lloyd/Siedge Weald scenario** because the session 28 FP list (12 tiles) came from an unreliable `cursor_walk` probe — those tiles may have been legitimate. Next session: enter Siedge Weald as Lloyd (Dragoon Mv4 Jmp4 + Mv+2 = 6), visually count blue tiles, compare to new BFS output, diff against session 28 FP list.
