...
- [~] **BattleMenuTracker desync — action menu navigation unreliable** [Movement] — Multiple symptoms observed 2026-04-11 (Chemist/Throw test). Partially fixed: battle_move, battle_attack, and battle_ability now read the actual cursor from `0x1407FC620` before navigating. Menu always has 5 items (Move becomes "Reset Move" after moving, never disappears — unless a movement ability like Manafont prevents reset). An earlier "fix" that shifted indices post-move was incorrect and reverted. Remaining issue: `battle_ability "Throw Shuriken"` entered Items instead of Throw — the ability-name → skillset resolver picks the wrong submenu. Still needs investigation.
