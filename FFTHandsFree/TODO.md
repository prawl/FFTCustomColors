...
- [ ] **JobSelection grid must account for unlocked-only cells per unit** — observed 2026-04-15 session 15 (Agrias). The in-game JobSelection grid only renders classes the unit has UNLOCKED; locked cells are either omitted entirely (grid collapses) or shown as inactive skip-overs. Current `JobGridLayout.ForUnit` assumes the FULL 6/7/6 grid for everyone, which is correct only for Ramza at Ch4 (or any other unit who has mastered all prerequisites). Repro: on Agrias (does not have Dancer unlocked), navigating 2 Downs + 4 Rights should reach Dancer at (2,4), but the game skips the locked cell, so the cursor ends up on a different class — and `layout.IndexToRowCol(heapByte)` produces a stale row/col because our layout still thinks (2,4) = Dancer. This also explains change_job_to misfires on under-leveled units.
  - Fix direction: feed the unit's unlock bitfield into `ForUnit`. Source: roster +0x32+jobIdx*3 bytes 0-1 hold per-job learned action abilities (see `project_roster_learned_abilities.md`). A job is "unlocked" when the unit meets its JP prerequisites (e.g. Archer requires Squire Lv 2) — prerequisites are in `CharacterData.JobUnlockRules` (not yet implemented; needs to be). Easier proxy: "has any learned action ability in this job's bitfield" → unlocked. Not perfect (a job can be unlocked without any learned abilities) but close. Best answer: read the game's own "job unlocked" flag directly if we can find it on the heap.
  - Once unlock is known, `ForUnit` returns a filtered grid where locked cells are omitted and flat-index math walks only unlocked cells. Row widths become per-unit.
  - Separate but related: record live-verified unlock rules (Squire → everything path) in CharacterData so we can predict what's unlocked without memory scans.
