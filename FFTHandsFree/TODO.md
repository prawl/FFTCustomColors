...
- [ ] **🔴 Victory misdetected as BattleDesertion** [Detection] — Live-repro captured 2026-04-25 Zeklaus Desert: solo-Ramza god-loadout killed last 2 enemies (one crystal, one by counter/attack), battle_wait returned `[BattleDesertion]` but `screen` immediately after showed `[WorldMap]` (Victory completed cleanly). Smoking-gun log line: `[BattleWait] Screen: BattleDesertion team=1 act=1 mov=1`. The `team=1` is stale battle-array slot-0 pollution where Ramza's slot read team=1 before the roster match corrected it to team=0 post-scan (logs: `Correcting team for (9,7): was 1, roster match → 0`). The raw byte feeding `ScreenDetectionLogic` reads `battleTeam=1` AT THE MOMENT detection runs, so the Desertion rule at line 518 (`postBattlePausedState && submenuFlag==1`) fires with team=1 even though the actual unit IS player-team. Fix path: (a) Use the roster-corrected team (not the raw battleTeam byte) when classifying post-battle states. (b) Require `battleTeam == 0` on the Desertion rule — real Desertion happens when a PLAYER unit left the field, so their team should be 0, not the stale 1. Test cases: Victory_with_kill + battle_wait should return `BattleVictory` / `WorldMap`, not `BattleDesertion`.
