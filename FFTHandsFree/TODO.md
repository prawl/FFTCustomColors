...
- [ ] **Compact view: collapse ValidPaths into one line** — `screen` compact output currently lists each available action on its own line (CursorUp, CursorDown, Select, OpenUnits, OpenInventory, OpenChronicle, OpenOptions, etc.) which burns vertical space. Should be one line like `Actions: CursorUp/Down, Select, Open{Units,Inventory,Chronicle,Options}` — grouped by prefix or comma-separated. Verbose view keeps the current format. Requested 2026-04-19 session 48.

- [ ] **WorldMap arrival: auto-tap C to snap cursor to current node** — When any flow returns to WorldMap (battle flee, minimap flee, location exit, etc.), the map cursor can end up a few tiles off the player's current location. Pressing C recenters the cursor on the player. Add a one-shot C tap after `WaitForScreen: "WorldMap"` transitions, or as a post-processor hook in the SM's WorldMap entry handler. Cheap, avoids a whole class of "why does travel target the wrong place" bugs. Requested 2026-04-19 session 48.
