...
### Session 28 — next-up follow-ups

- [ ] **Fix MovementBfs — 12 known false positives for Lloyd at Siedge Weald** — 2026-04-17 session 28 live data. Lloyd (Dragoon Move=4 Jump=4 + Movement+2 ⇒ effective Move=6) at (10,9) on MAP074. User visually counted 20 blue tiles, BFS returned 16, `cursor_walk` flood-fill (partial, 5 tiles reached) flagged these 12 tiles as BFS-said-valid/game-rejected: `(10,6), (9,7), (10,7), (8,8), (9,8), (6,9), (7,9), (8,9), (7,10), (8,10), (8,11), (9,11)`. These are the **diagnostic targets for fixing BFS**: step through each tile's path through `ColorMod/GameBridge/MovementBfs.cs` and the `ColorMod/GameBridge/MapLoader.cs` height/slope model and find why BFS thinks they're reachable. Canonical PSX movement rules are well-documented (GameFAQs/FFTHackery); best next step is to web-search the reference algorithm and port it rather than reverse-engineer from probes.
...
- [ ] **JobCursor resolver: find a byte that passes liveness on this save** — session 27 added a liveness probe to `ResolveJobCursor` that catches widget-counter candidates that track oscillation but not real nav. On the current save, ALL 32 candidates failed liveness (see `memory/project_jobcursor_liveness.md`). Drift correction is plumbed but can't fire without a live-tracking byte. Next-session approaches: (a) try heap_snapshot instead of module-memory snapshot (widget state lives in UE4 heap), (b) narrow the 2-step verify to +1 AND +1 (not +1 and +2) in case the byte is a "changed count" rather than absolute position, (c) try multi-key-sequence verify (Right×3, expect +3) to catch candidates that only respond to the first key.
