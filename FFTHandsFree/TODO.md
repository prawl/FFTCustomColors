...
- [ ] **Per-job equippability table** — added 2026-04-15 session 16. Blocker for `ui=<hovered item>` and `availableWeapons[]` on EquippableWeapons picker. Need a static table mapping each job → set of weapon/shield/helm/armor/accessory types (and specific items where restricted — e.g. Cloud only equips Materia Blade). Source: FFTPatcher `Jobs.xml` has per-job equippable lists; port to C# alongside `CharacterData.cs`. Once shipped, combine with `ItemData.cs` (type + owned filter) to generate the correct in-game picker list for any (unit, slot, tab) triple. Also feeds `change_right_hand_to <name>` validation.
...
Items that are fully shipped (checked `[x]`). Moved here 2026-04-15 session 16 so the active TODO stays focused on whats still in flight. Partial (`[~]`) items stay above in their original section.
