...
- [ ] **battle_wait without move/act should prompt confirmation** [Execution] — If Claude calls `battle_wait` without having moved or used an ability, respond with "Are you sure? Send battle_wait again to confirm." Prevents accidental wasted turns. Check `acted`/`moved` flags before executing.
...
- [ ] **ui= field should show the selected ability in Battle_Attacking/Casting** [Movement] — When entering targeting mode, `ui=` still shows "Abilities" instead of the ability being cast (e.g. `ui=ThrowStone` or `ui=Fire`). Claude needs to confirm which ability is active — especially after the submenu navigation bugs.
...
- [ ] **battle_ability self-target leaves game in moving state** [Execution] — `battle_ability "Chakra"` on Lloyd (Dragoon) reported Battle_Casting/ui=Chakra but the game was actually in the moving state. The command timed out and the screen detection was wrong. Needs investigation: either the self-target confirmation path isn't completing, or screen detection is misreading the moving state as casting. Observed 2026-04-12.
