...
- [ ] **Valid target tiles — self-radius abilities** [AoE] — Cyclone, Chakra, Purification, Shout have `HRange="Self"` with `AoE>1`, meaning the splash is centered on the caster. No target-picking required; just emit the affected tiles directly. Currently the calculator rejects them as ineligible. Single special case to add.
- [ ] **Valid target tiles — full-field abilities** [AoE] — All Bardsong (Seraph Song, Life's Anthem, Finale…) and all Dance abilities hit every unit of the target type on the map. No tile list needed — just mark these as "affects all allies" or "affects all enemies" so Claude knows range is irrelevant.
...
- [ ] **battle_move/battle_ability should return updated state** [Execution] — After `battle_move`, `battle_attack`, or `battle_ability` succeeds, the response should include the unit's new position, updated HP, and any immediate effects (damage dealt, status applied) so Claude can confirm the action worked without an expensive `scan_move` rescan. Currently these commands return validPaths but no post-action state. At minimum: `{movedTo: [x,y], hp: N, acted: true}`. Ideally also invalidate the scan cache so the NEXT scan is fresh if Claude does need full state.
- [ ] **Invalidate scan cache after battle_ability** [Execution] — After using an ability (X-Potion, Throw, Fire, etc.), the scan cache still holds pre-ability data. The next `scan_move` returns stale HP values, so Claude can't verify the heal/attack worked. `battle_ability` should invalidate the TurnAutoScanner cache on completion so the next scan is fresh. Related to the existing "Verify attack landed" item below — both need a post-action rescan.
...
- [ ] **BattleMenuTracker desync — action menu navigation unreliable** [Movement] — Multiple symptoms observed 2026-04-11:
  1. `battle_move 8 7` navigated into Abilities submenu instead of Move mode
  2. `battle_ability "Throw Shuriken"` entered Items instead of Throw
  3. `battle_ability "Throw Stone"` after a successful `battle_move` just moved the cursor to Abilities and stopped instead of entering the submenu
  
  Root cause: the BattleMenuTracker's internal cursor position doesn't match the game's actual menu cursor. After `battle_move` completes, the cursor returns to the action menu but the tracker doesn't know where it landed. Subsequent commands assume a starting position that's wrong, so arrow-key navigation ends up on the wrong menu item. The tracker was marked as "[~] known issue" in Tier 2 but it's blocking basic gameplay — nearly every `battle_move` → `battle_ability` sequence in testing hit this bug. Needs a reliable way to sync: either read the menu cursor address (`0x1407FC620`) before navigating, or always navigate from a known position (e.g. press Up 4x to guarantee Move is selected).
...
- [ ] **Non-active player unit job names can be wrong** [Identity] — Kenrick (a Knight) displayed as "White Mage" in scan_move output. The job name for non-active player units likely comes from the UI buffer or roster job byte, which can be stale or mapped to the wrong class after C+Up cycling. Only the active unit's job is reliable (from the condensed struct). Non-active allies need a more reliable source — possibly the roster's per-slot job byte at +0x02, or the heap struct fingerprint if generic player units have distinctive fingerprints. Observed 2026-04-11 with a 4-unit party.
