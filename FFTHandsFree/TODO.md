...
- [ ] **JobSelection cursor reports (0,0) after row/col navigation on the same visit** — observed 2026-04-15 session 15 (Agrias). Repro: open Agrias JobSelection, call `screen` (triggers resolver, reports `ui=Holy Knight`, cursor (0,0)). Send 2 Downs + 4 Rights to land on row 2 col 4 (Dancer). Call `screen`. Expected: `ui=Dancer`, `cursorRow=2`, `cursorCol=4`. Actual: `ui=Holy Knight`, `cursorRow=0`, `cursorCol=0`. Screenshot confirmed the IN-GAME cursor moved correctly to Dancer — so it's the resolved heap byte read returning stale/zero after the multi-key nav. Hypothesis: the resolver's cached heap address was invalidated by the widget re-render that happens when the picker cursor crosses rows, OR the reads raced a pending repaint. Works fine for horizontal-only movement within the same row (confirmed earlier with Ramza). Workaround: call `resolve_job_cursor` action manually after crossing rows. Real fix candidates: (a) revalidate the cached address on every read by re-oscillating cheaply — too expensive, defeats the cache; (b) track a "dirty" flag set when a Down/Up key is observed, forcing a re-resolve on the next screen call; (c) read a stable pointer/vtable anchor for the cursor widget so the address doesn't drift. (b) is the pragmatic next step.
