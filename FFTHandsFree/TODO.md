...
- [ ] **Decode row index to item name** — with the cursor index known, resolve `ui=<item name>` by looking up the row's position in the shop's stock list. Partial data found 2026-04-14:
  ...
  - **In-process scraper approach (recommended next session):** broad-search for UE4 FStrings (`80 92 A2 36 FD 7F 00 00` vtable) is cheap when restricted to UE4 heap regions (0x100000000–0x200000000). A new C# action `scrape_shop_items` could: (1) walk those regions in one pass, (2) for each FString whose vtable matches and whose length is plausible (4-40 bytes UTF-16), read its contents, (3) compare against the master weapon-name pool at `0x6007BE4`, (4) return the set of items present as FStrings in the live heap, sorted by address. The master pool already contains every item name in the game, so no external database needed. Round-trip would be a single action call (~1s), beating the 100+ search_all calls the bridge would otherwise need.
