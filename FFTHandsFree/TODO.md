...
- [x] **PartyMenu cursor state-machine drift on Escape from CharacterStatus** — FIXED 2026-04-15 session 15. Live verification revealed the game ALWAYS snaps PartyMenu cursor to (0,0) on Escape from CharacterStatus, regardless of where the unit was selected from. State machine used to restore the saved cursor (`_savedPartyRow/Col`), which was permanently desynced from reality. Fix: `HandleCharacterStatus` Escape now snaps `CursorRow`/`CursorCol` to (0,0) instead. `_savedPartyRow/Col` are still used by `ViewedGridIndex` while on nested screens so the viewed unit stays consistent until Escape.

- [ ] **JobSelection auto-resolver race during screen transitions** — observed 2026-04-15 session 15. Repro: from a fresh PartyMenu, Down → Enter → Down (sidebar) → Enter (JobSelection) in a single batched command. State machine flips to JobScreen on the second Enter, then `DetectScreen` may run before the game has finished its open-animation. The JobSelection auto-resolver fires its 6 Right/Left keys, which the game (still in PartyMenu transition) interprets as PartyMenu navigation → cursor drifts. Effect: after the resolver fires, in-game cursor can be on a different unit than state machine's saved viewed-unit, so `ui=` and `viewedUnit=` are wrong on the next screen call. Fix candidates: (a) delay the auto-resolver until the screen has been stable for N consecutive DetectScreen calls (currently fires immediately); (b) require a memory-confirmed JobSelection signal before resolving (e.g. the menuDepth byte at 0x14077CB67 reads >0 in nested panels — verify this address doesn't lie during transition); (c) skip the auto-resolver entirely and require explicit `resolve_job_cursor` calls (worst UX). (a) is the cheap version.
