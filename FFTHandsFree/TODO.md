...
- [x] **🎯 Move/Jump stat read: bypass UIBuffer, read heap unit struct** — shipped session 29 pt.11. `TryReadMoveJumpFromHeap` in NavigationActions.cs searches the UE4 heap for each active unit's HP/MaxHP pattern (same technique as ClassFingerprint lookup) and reads Move at struct+0x22, Jump at struct+0x23. Live-verified on MAP074:
  - Kenrick (Knight)  HP=586 → +0x22=03 +0x23=03 = Mv=3 Jmp=3 ✓
  - Lloyd (Dragoon)   HP=549 → +0x22=05 +0x23=04 = Mv=5 Jmp=4 ✓
  - Archer (enemy)    HP=453 → +0x22=03 +0x23=03 = Mv=3 Jmp=3 ✓
  - Net effect: scan_move now auto-matches game tile counts across all tested units (Kenrick (9,9)=11, Kenrick (8,5)=15, Kenrick (8,2)=19, Wilham (10,11)=5). No manual override needed. Falls back to UIBuffer if the heap search fails. Diagnostic infra: new `dump_unit_struct` bridge action dumps 256 bytes of any unit's heap struct for future offset hunts.

- [ ] **MovementBfs (10,4) FP — investigate climb-up rule (stale until Move/Jump bug above is fixed)** — Session 29 live-verified Lloyd (real Mv=5 Jmp=4 per user; BFS was using Mv=6 Jmp=4 due to the stat bug above) at (10,9) on MAP074 Siedge Weald. With stale Mv=6, BFS returned 21, game showed 20, FP was (10,4). This may disappear once Mv=5 is used. Re-test after the Move/Jump heap-read fix lands. The prior session-28 FP list (12 tiles) was WRONG — `cursor_walk` probe miscounted — don't chase that list again.

- [ ] **Rescue `cursor_walk` reliability (still finds ~5 of 20 tiles)** — 2026-04-17 session 28 pt.7 shipped the `cursor_walk` action. The per-probe "is cursor on valid tile" signal — counting `04 → 05` transitions in `0x140DDF000..0x140DE8000` — only catches 5 of 20. Fix ideas: (a) widen range to cover the mirror region `0x140F9xxxx`, (b) count more transitions (`00 → 05`, `01 → 05`), (c) run baseline "cursor on known valid tile" and compare the SET of `0x05` bytes. Blocking automated BFS regression fixture building.

- [ ] **Find a byte (or compound signal) that actually encodes the game's valid-tile count** — 2026-04-17 session 28 live evidence: `0x142FEA008` reads `0x0B` (11) while the user visually counts 20 blue tiles. The byte we believed encoded the game's tile count does NOT. `LogBfsTileCountMismatch` is commented out in `CommandWatcher.cs` (`case "scan_move"` branches; search `LogBfsTileCountMismatch`). Plan: get a user-confirmed visual count for Lloyd/Siedge Weald + 2–3 other unit/map combos; snapshot module memory before/after Move mode entry; scan the diff for any byte or u16 whose post-entry value equals the real count on ALL combos. If no single byte matches, consider a compound signal (sum of `+0x06 == 0x04` in the render-slot range, etc.). `MoveTileCountValidator` + `DetectedScreen.BfsMismatchWarning` + shell render are all in place waiting for the real signal.

- [ ] **battle_move false-negative: diagnose next live repro** — Session 29 pt.4 added richer timeout logging (`[MoveGrid] TIMEOUT: 8s elapsed without confirm. lastScreenSeen=X polls=N`). Next time the bug fires, the error message will tell us what screen state the detector saw for the 8s window. Hypothesis to test with that data: `battleMode` byte stays at 2 past the walk animation because UE4 keeps painting Move UI briefly. If `lastScreenSeen=BattleMoving` across all 80 polls, the fix is in screen detection (need a second signal to tell "Move UI is up but unit already moved"). If intermediate states appear, the fix is in the accept-list.

- [ ] **Live-verify activeUnitSummary on all Battle\* states** — Session 29 pt.1/2 shipped `[BattleMyTurn] ui=Move Lloyd(Dragoon) (10,9) HP=628/628 MP=52/52`. Confirmed on BattleMyTurn and BattleAttacking(manual) during session 29. Still needs quick verification on BattleMoving / BattleCasting / BattleAbilities / BattleActing / BattleWaiting / Battle_\* sub-screens — the rendering branch widened to `Battle*` so all should work but unverified. Also note: `execute_action` response uses a separate compact renderer (fft.sh `_fmt_execute_response`) that does NOT use activeUnitSummary — see sibling TODO if that's worth unifying.

- [ ] **execute_action response missing activeUnitSummary** — Session 29 pt.2 widened `_fmt_screen_compact` to render activeUnitSummary on Battle\* states, but `_fmt_execute_response` (fft.sh:~2543) is a separate renderer that still extracts `activeUnitName`/`activeUnitJob` raw fields and doesn't include position/HP. Live-observed during session 29: `screen` shows `Lloyd(Dragoon) (10,9) HP=628` but the very next `execute_action Cancel` response shows only `ui=Abilities`. Either unify the two renderers or mirror the activeUnitSummary render into `_fmt_execute_response`.

- [ ] **Live-verify heldCount rendering on Items abilities** — Session 29 pt.2 shell edit adds `Potion [x4]` / `Ether [OUT]` annotations to `fmtAb` in fft.sh:~3122. Verified compiles + tests pass but not live-verified. Needs an Items user in an active battle (Ramza or Chemist) to confirm.

### Session 28 — next-up follow-ups (archive)

- [x] **Fix MovementBfs — 12 known false positives for Lloyd at Siedge Weald** — Session 29 pt.5/6 ported TileEdgeHeight into MovementBfs. Live verification against the FP list moved to session-29 follow-up above. Historical context below kept for reference.
  - 2026-04-17 session 28 live data. Lloyd (Dragoon Move=4 Jump=4 + Movement+2 ⇒ effective Move=6) at (10,9) on MAP074. User visually counted 20 blue tiles, BFS returned 16, `cursor_walk` flood-fill (partial, 5 tiles reached) flagged these 12 tiles as BFS-said-valid/game-rejected: `(10,6), (9,7), (10,7), (8,8), (9,8), (6,9), (7,9), (8,9), (7,10), (8,10), (8,11), (9,11)`.

- [ ] **Find a byte (or compound signal) that actually encodes the game's valid-tile count** — 2026-04-17 session 28 live evidence: `0x142FEA008` reads `0x0B` (11) while the user visually counts 20 blue tiles. The byte we believed encoded the game's tile count does NOT. `LogBfsTileCountMismatch` is commented out in `CommandWatcher.cs` (`case "scan_move"` branches; search `LogBfsTileCountMismatch`). Plan: get a user-confirmed visual count for Lloyd/Siedge Weald + 2–3 other unit/map combos; snapshot module memory before/after Move mode entry; scan the diff for any byte or u16 whose post-entry value equals the real count on ALL combos. If no single byte matches, consider a compound signal (sum of `+0x06 == 0x04` in the render-slot range, etc.). `MoveTileCountValidator` + `DetectedScreen.BfsMismatchWarning` + shell render are all in place waiting for the real signal.

- [ ] **`cursor_walk` probe reliability — currently finds ~5 of 20 valid tiles** — 2026-04-17 session 28 pt.7 shipped the `cursor_walk` action (`CommandWatcher.RunCursorWalkDiagnostic`). Arrow-key calibration works and auto-recalibrates when edge-blocked. Flood-fill navigation works. But the per-probe "is cursor on valid tile" signal — counting `04 → 05` transitions in `0x140DDF000..0x140DE8000` between pre/post snapshots — only catches 5 of 20 valid tiles for Lloyd at Siedge Weald. The slot region bounds may be too narrow, OR the transition pattern differs for tiles not in the primary cluster, OR slot reuse hides the transition for some tiles. Fix ideas: (a) widen range to also cover the mirror region `0x140F9xxxx`, (b) count transitions of `04 → 05` AND `00 → 05` AND `01 → 05`, (c) run a baseline "cursor on known valid tile (unit's own)" and compare the SET of 0x05-bytes that appear, rather than transition counts.

- [x] **`screen` / `BattleMyTurn` response should include the active unit's name** — shipped session 29 pt.1/2 via `ActiveUnitSummaryFormatter` + widened `Battle*` branch in fft.sh. Live-verified 2026-04-17 at Siedge Weald: `[BattleMyTurn] ui=Move Lloyd(Dragoon) (10,9) HP=628/628 MP=52/52`. execute_action renderer still needs parity — see session 29 follow-up. Original description preserved:
  - 2026-04-17 session 28 live at The Siedge Weald. `screen` on `BattleMyTurn` reports only `[BattleMyTurn] ui=Move curLoc=... obj=...`. Whose turn is it? Unknown from the response alone — Claude has to guess or `scan_move` (~heavy). The active unit's name (and ideally job / HP / MP) is the single most useful piece of context for deciding the next action. `scan_move` already surfaces this (`Wilham(Monk) (10,10) HP=477/477 MP=115/115`) — it should appear on the compact `screen` line too for any `BattleMyTurn` / `BattleActing` / `BattleMoving` / `BattleAttacking` / `BattleAbilities` / `Battle_*` state. Example desired compact format: `[BattleMyTurn] Wilham(Monk) (10,10) HP=477 ui=Move`. Likely lives alongside the existing `ui=` populator in `ScreenDetectionLogic` / `CompactScreenFormatter`.

- [~] **`battle_move` reports "NOT CONFIRMED (timeout)" but the unit actually moved** — session 29 pt.4 added diagnostic logging so the next live repro will include `lastScreenSeen=X polls=N` in the timeout message. Fix still pending on that data. Original description preserved:
  - 2026-04-17 session 28 live-repro. Called `battle_move 10 9` with Wilham at (10,10). Helper returned `[BattleMoving] failed: (10,10)->(10,9) NOT CONFIRMED (timeout)`. BUT: immediately after, mod logs show `Unit=(10,9)` — so the move *succeeded* in-game. The game registered the move, the cursor-confirmation poll missed it, and the shell reports `BattleMoving` still (because the `F` confirm didn't advance state machine OR the state machine didn't observe the transition in time). Claude then assumes failure and tries to recover, but the turn is already half-consumed. Two possible root causes: (a) F-key timing — press happens, but the poll window closes before the game's transition animation reaches the state-machine; (b) state-machine's notion of "confirmed" requires a different signal than what the game emits on a same-tile-move. Screenshot at `screenshot_20260417_091931.png` confirms cursor was positioned on target tile, game was still rendering move selection UI. Reproduces reliably on turn 1 after `auto_place_units` settles. Captured during session 28 battle-task verification pass.

- [x] **`ui=` missing from `BattleAttacking` compact response** — shipped session 29 pt.3 via `TargetingLabelResolver`. Falls back to BattleMenuTracker's selected ability/item when `_lastAbilityName` is null (manual-nav entry path). Live-verified: `[BattleAttack] ui=Attack` after `execute_action Abilities → Select`. Original description:
  - 2026-04-17 session 28 live-verify. `BattleMyTurn ui=Move` ✅, `BattleAbilities ui=Attack` ✅, `BattleMoving ui=(x,y)` ✅ (fixed in pt.2), `BattleAttacking` **no ui** ❌. Code path exists in `ScreenDetectionLogic.DetectScreen` — `BattleAttacking` requires `_lastAbilityName` to be set (only populated when entered via `battle_ability`/`battle_attack` helpers, not via manual `execute_action Select` from BattleAbilities). Fix: when transitioning from `BattleAbilities` → `BattleAttacking`/`BattleCasting`, copy `_battleMenuTracker.CurrentAbility` into `_lastAbilityName`.
...
- [x] **Wire the dry-run nav harness into the real `NavigateToCharacterStatus`** — `NavigateToCharacterStatus` at NavigationActions.cs:4528 already consumes `NavigationPlanner.PlanNavigateToCharacterStatus` (live method delegates to planner, single source of truth). Session 29 pt.12 added the missing piece: shell helpers `open_character_status` / `open_eqa` / `open_job_selection` now accept a trailing `dry-run` arg that routes to the `dry_run_nav` bridge action instead, printing the planned key sequence without firing. Example: `open_eqa Agrias dry-run`.

- [ ] **JobCursor resolver: find a byte that passes liveness on this save** — session 27 added a liveness probe, session 29 strengthened it to a 3-step Right/Left probe (approach (c) from session 27's list). On the current save, ALL 32 candidates failed liveness (see `memory/project_jobcursor_liveness.md`). Drift correction is plumbed but can't fire without a live-tracking byte. Remaining approaches to try: (a) different heap snapshot timing (maybe the byte settles AFTER the 700ms we wait), (b) bidirectional verify (Right once, Left once, byte should return to baseline — rules out one-way change-count widgets), (c) JobSelection entry triggers some widget update that shuffles the heap — try resolving AFTER a Down/Up to stabilize. The session 29 3-step probe is stricter than the old 1-step; may now reject candidates that the 1-step accepted as well.

- [ ] **EqA row resolver: re-fire on detect-drift events** (session 24 carryover) — auto-resolver already fires once on EqA entry (`_eqaRowAutoResolveAttempted` latch, session 24 pt.3) and sets `ScreenMachine.SetEquipmentCursor(resolvedRow)`. But it ONLY runs once per entry. If the SM drifts DURING an EqA session (e.g. after a picker open/close), the stale cursor persists. Consider re-firing the resolver on detect-drift events (menuDepth re-read after a picker exit).
...
- [x] **Orlandeau primary skillset label** — already fixed at some point: `GetPrimarySkillsetByJobName` in CommandWatcher.cs:3693 maps `"Thunder God" => "Swordplay"`. Verified 2026-04-17 session 29 (TODO was stale). Sub-skillset detection (Holy/Unyielding/Fell Sword per cursor) NOT currently implemented — defer until a live use case surfaces.
...
- [x] **JP Next: populate Mettle/Fundaments ability costs** — already done. AbilityJpCosts.cs:29 populates Focus/Rush/Throw Stone/Salve (Fundaments) and Tailwind/Chant/Steel/Shout/Ultima (Mettle). Values are wiki-sourced since the reference save has all maxed; still awaits live-verify when an unlearned-Mettle unit surfaces.
