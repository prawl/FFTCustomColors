...
- [ ] **Inventory quantity for Items and Throw** [Abilities] — Both Items and Throw abilities consume the caster's inventory. In-game the submenu shows a "Held" column (Potion=3, High Potion=0, X-Potion=93) and Throw lists one entry per weapon type with its held count (Dagger=1, Mythril Knife=2). Our scan_move currently lists every ability in the skillset as if unlimited. Claude can't tell High Potion is unusable (Held=0), can't know Throw Dagger only works once before running out, and can't decide between throwing a Mage Masher (1 held) vs a Mythril Knife (2 held). Need to find the per-character inventory array and surface each item's held count alongside the ability entry (`held: N` field), OR emit a single `heldCount` field per ability that's only populated for inventory-consuming skillsets (Items, Throw). Look for the inventory data — likely in the roster slot somewhere, or possibly a separate global inventory for Items (shared party stash) vs per-character for Throw. Once we know a Potion has Held=0, filter it out of validTargetTiles or mark it `unusable: true` in JSON.
