...
- [~] **Viewed-unit struct on EquipmentAndAbilities** — 2026-04-14 located a full condensed unit struct that mirrors the viewed unit's stats in one contiguous block. Byte pattern scan for `CF 02 CF 02` (Ramza HP=MaxHP=719 as u16 pair) returned exactly one heap match at a session-specific address (0x4166C2E030 in this session). The struct begins 0x30 bytes before that match; at +0x0C it replicates the roster record byte-for-byte (spriteSet/unitIndex/job at block+0x0C..+0x0E, equipment u16 slots at block+0x1A..+0x26, HP/MP pairs at block+0x30..+0x37). So everything we need to show per-unit loadouts IS in this one struct.
  - **Blockers:** (1) The struct address is heap and drifts on every mod restart. (2) No static pointer chain found — searched for pointers-to-the-struct and pointers-to-those-pointers; chains dead-end in anonymous UE4 objects. (3) A shared vtable at `0x7FFDCE049280` (exe) appears in multiple wrappers so it doesn't uniquely identify the hovered-unit wrapper.
  - **Viable next-session paths:**
    - **(a) AoB rediscovery by matching roster bytes**: scan `0x1411A18D0` (50 roster slots × 0x258) and AoB-search for slot 0's bytes in the scannable heap; the unique match is the hovered-unit struct's roster mirror. Re-run on every PartyMenu entry (cheap, a few ms).
    - **(b) Q-cycle diff on each session**: after entering EquipmentAndAbilities, auto-cycle E twice while snapshotting, diff to find the viewed-slot byte, then read forever within the session. One-time cost per session.
    - **(c) Static exe AoB for the containing widget**: search exe code for the instruction that writes the viewed-slot byte, then follow back to the data symbol. Robust but slow to set up.
  - Recommended path for next session: **(a)** — it's the simplest and self-correcting. Also yields the full struct (HP/MP/equipment) not just slot index. Until then, loadout surfacing identifies Ramza (slot 0) only via state-machine IsRamza flag.
