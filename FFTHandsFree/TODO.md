...
- [~] **`battle_ability <name> <x> <y>`** [Abilities] — Navigate Abilities menu → select a specific ability → target a tile → confirm. Self-target (Shout, Focus) and targeted (Pummel, X-Potion) verified working. Menu navigation now reads actual cursor from `0x1407FC620` and accounts for the post-move menu index shift (Abilities is index 0 after moving, index 1 before). Still needs live testing to confirm the index-shift fix works in all cases. BattleMenuTracker state tracking is partially replaced by direct memory reads.
