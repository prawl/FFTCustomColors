...
- [ ] **AttackOutcomeClassifier reports MISS on a hit that KO'd the target** [Bridge/Outcome] — Live-flagged 2026-04-26 playtest: Kenrick attacked Summoner at (6,4); the attack KO'd the Summoner (visually confirmed in-game), but the bridge response reported `> [battle_ability] Attacked (6,4) from (8,4) — MISSED!`. The classifier's logic likely compared pre-attack target HP vs post-attack target HP, but if the post-attack scan couldn't find the target (because they died and dropped from the live unit list / their tile cleared), the classifier interpreted "no HP delta found" as a miss instead of a kill. Fix: when target unit is missing from post-snapshot but their pre-snapshot HP was positive, attribute the result as a kill (HP→0) not a miss. AttackOutcomeClassifier lives at `ColorMod/GameBridge/AttackOutcomeClassifier.cs`. Cross-ref also `feedback_phantom_success_pattern.md` (the inverse — saying "hit" when no damage dealt). Both directions need post-action verification with multiple HP sources.

- [ ] **False phantom-KO narrator events from transient empty-scan during diff** [Bridge/Narrator] — Live-flagged 2026-04-26 playtest: narrator emitted `> Time Mage took 345 damage (HP 345→0)` + `> Time Mage gained Dead` + `> Time Mage lost Haste` immediately followed in the same batch by `> Time Mage (ENEMY) joined at (3,4)`. Time Mage was alive throughout — no actual KO happened. Pattern: a transient empty / partial scan during the wait poll dropped the unit from the snapshot; `UnitScanDiff.Compare` interpreted "alive HP=N → missing" as a KO event; the next scan re-introduced the unit, narrator emitted "joined." Same shape as the `feedback_cache_preserve_on_null_active_unit.md` issue but for full-roster diffs not active-unit cache. Fix: require unit identity to actually transition to HP=0 in a STABLE post-snap (re-read once, confirm) before emitting `damaged`/`ko`/`joined` events. OR: when a unit appears to leave the scan, hold the death event for one tick and check the next scan — if they're back, suppress the phantom event. The `project_scan_diff_identity_collision.md` memory note covers a related case (duplicate-name enemies emitting remove+add). This phantom is the same family.
...
- [ ] **Post-victory screen detection mis-classifies WorldMap as LoadGame** [Bridge/Detection] — Live-flagged 2026-04-26: after Siedge Weald battle ended `BattleVictory`, called `execute_action Advance`. Game visually returned to the WorldMap, but the bridge detected the screen as `LoadGame` (showing `ValidPaths: ScrollUp, ScrollDown, Select, Cancel` — the load-save-file picker validPaths). Likely a memory-fingerprint collision: WorldMap and LoadGame share enough of the detection signature post-battle that the classifier picks the wrong one. Cross-ref `project_worldmap_vs_travellist_byteidentical.md` (similar collision resolved via SM override). Look for an SM override path or an additional discriminator byte that distinguishes post-battle WorldMap from LoadGame. Worth writing a regression test that pins the post-Victory→Advance→WorldMap path explicitly. Also check the `Advance` validPath impl — it may be advancing through more screens than intended (post-victory dialog → EXP → etc.) and overshooting into a LoadGame that wasn't actually open.

- [ ] **Phantom-success: ability "lands" but the game never received the target** [Bridge/Timing] — Live-flagged 2026-04-26 playtest: Ramza X-Potion → (10,8) (Wilham at 8 HP). Bridge response: `> [battle_ability] Used X-Potion on (10,8)` and turn advanced. But Wilham was NOT actually healed — user observed the input was too fast for the game UI to register the target selection. Distinguishes from the static-region-stale-HP bug above (where heal landed but scan was stale): here, the heal genuinely never happened. Likely cause: between submenu nav, target-tile nav, and final Enter-confirm, one of the key intervals is too short and the game dropped a press; the bridge interprets "no error during nav" as success. Fix: post-action verification on heal/damage abilities — read target HP before AND after, classify miss-vs-stale vs phantom: (a) HP changed = success; (b) HP unchanged + we waited a settling window = real failure (don't claim success). Tie-breaker: read MaxHP/Status (e.g. Critical for low-HP) — success should also clear Critical when applicable. Cross-ref: `feedback_phantom_success_pattern.md` memory note covers similar shape.
...
- [ ] **Surface "can't act this turn" disable-state on the active-unit header** [Bridge/Render] — when Wilham's the active unit but has `[DontAct]` (or `[Sleep]`, `[Stop]`, `[Petrify]`, `[Charm]`, `[Confusion]`, `[Frog]`, `[Chicken]`, etc. — anything that prevents normal action), the header just reads `[BattleMyTurn] ui=Move Wilham(Samurai) (9,8) HP=394/528 ...`. The disable info IS in the bottom Units block as `[Haste,DontAct]` but the agent has to scroll/grep to find it for the active unit specifically. The render already surfaces `acted` / `moved` tags on the header line when `battleActed=1` / `battleMoved=1` (see [fft.sh:588-589](fft.sh#L588) — `LINE="$LINE $(_col "$_C_WARN" "acted")"`). Add an analogous `disabled=DontAct` (or `disabled=Sleep`, etc.) tag pulled from the active unit's StatusBytes when any action-blocking status is set. **Live-flagged 2026-04-26 playtest #9**: agent (and dev) both missed Wilham's DontAct status, agent kept retrying menu-nav and accidentally landed on AutoBattle. Action-blocking statuses to detect: DontAct, DontMove, Sleep, Stop, Petrify, Frog, Chicken, Confusion, Charm, Berserk, Performing, Charging (no new action), Jump (mid-jump). Distinguish "can't act" from "can't move" cleanly: agent might still be able to wait+heal even if DontAct; can't even move if Sleep/Stop/Petrify. Suggested format: `[BattleMyTurn] ui=Move Wilham(Samurai) (9,8) HP=394/528 ... DontAct(can't act, can wait)`.
