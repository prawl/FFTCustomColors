...
- [ ] **🎯 Move/Jump stat read: bypass UIBuffer, read heap unit struct** — Session 29 live data surfaced the root cause of repeated BFS tile miscounts: `scan_move` reads active-unit Move/Jump from UIBuffer at `0x1407AC7C0 +0x24/+0x26`, but UIBuffer holds the CURSOR-hovered unit's stats, NOT the active unit's. Also: even when fresh, UIBuffer shows BASE stats (e.g. Knight Mv=3 Jmp=3) — equipment/ability bonuses not applied (memory/project_battle_bfs_verified.md, project_memory_scan_results.md).
  - **Verified live:** Kenrick (Knight, true Mv=3 Jmp=3) → scan_move logs `move=4 jump=4` → BFS returns 16 tiles vs game's 11. Wilham (Monk, true M=3 J=4) → scan_move reads different values. Ramza M=4 J=5 (confirmed from user). Lloyd M=5 J=4 (confirmed).
  - **Root cause code:** `NavigationActions.cs:3668` sets `unit.Move = (int)activeReads[12]` / `unit.Jump = activeReads[13]` only when `isActive` fires, and reads from UIBuffer.
  - **Fix path:** heap unit struct (found via HP/MaxHP search at `minAddr: 0x4000000000L`, struct base = match_addr - 0x10, class fingerprint at +0x69) is the canonical per-unit data source. Find Move/Jump offsets in that struct via diagnostic dump: for each active unit, log 128 bytes starting at struct base + 0x20 (after HP/MP block), grep for bytes matching true Mv/Jmp value. Then add those offsets to the existing batch read alongside ClassFingerprint.
  - **Impact on BFS accuracy:** probably the #1 cause of all the session 28/29 tile-count mismatches. Until fixed, users must pass `scan_move <mv> <jmp>` manually. Session 29 pt.8 unification (scan_move → MovementBfs) means fixing Move/Jump source fixes both code paths at once.
