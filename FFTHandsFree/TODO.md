...
- [ ] **battle_move navigates into Abilities submenu instead of Move mode** [Movement] — `battle_move 8 7` from Battle_MyTurn (Ramza as Chemist/Throw) landed on `Battle_Abilities ui=Attack` — the mod opened the Abilities submenu and put the cursor on Throw instead of entering Move mode. Likely a BattleMenuTracker state-machine bug where the "navigate to Move in action menu" path picks the wrong menu index. Reproduces consistently from the battle start on Zeklaus Desert. Observed 2026-04-11 during the Chemist/Throw gameplay test.
...
- [ ] **battle_ability picks wrong skillset when ability name is disambiguous** [Abilities] — `battle_ability "Throw Shuriken" 8 8` (Ramza as Chemist/Throw) navigated into the **Items** submenu instead of **Throw**, then targeted (8,8) which was out of Items' R:4 range and failed. The ability-name → skillset resolver is matching to the wrong submenu. May be caused by case-insensitive substring matching, or by always picking the first learned skillset regardless of where the ability lives. Needs to look up the ability name → skillset from `ActionAbilityLookup.GetSkillsetForAbilityId` (or a name-based equivalent) BEFORE entering the Abilities submenu, then navigate to the correct skillset. Observed 2026-04-11 during the Chemist/Throw gameplay test.
