...
- [ ] **Multi-sample debounce + transition state machine for screen detection** [Bridge/Detection] — Root cause of every "screen XYZ mis-tagged" bug we've shipped: `ScreenDetectionLogic.Detect()` reads ~12 disconnected memory bytes (paused, battleMode, encA/B, slot0/9, eventId, gameOverFlag, etc.) non-atomically and runs a long if-else cascade. During every transition there's a window where some bytes advanced but others haven't, producing fingerprints that match the wrong screen. The current pattern of "add another guard to the rule" creates new edge cases as fast as it closes them. Two cheap wins layered together: (1) **multi-sample debounce** — when `Detect()` returns a screen that differs from `_lastClassifiedScreen`, immediately re-read and re-classify; only commit when 2-of-3 reads agree (within ~200ms total). Kills the transient flicker that drives Victory→LoadGame, Victory→Desertion, Cutscene→WorldMap, AlliesTurn→CrystalMoveConfirm misfires. (2) **Transition state machine** — for each screen, declare which screens it can legitimately transition TO (e.g. BattleAttacking can go to BattleMoving/BattleActing/BattleVictory/GameOver/BattleAttacking, NOT directly to BattleCrystalMoveConfirm). Reject implausible transitions and force a re-sample. Combined: addresses the family of bugs without per-bug rule patches. ~1 day of work, no game knowledge required. Cross-ref the open detection-mis-tag bugs in §9.

- [ ] **Post-victory transient BattleDesertion mis-tag** [Bridge/Detection] — Live-flagged 2026-04-26 playtest at Siedge Weald: after KO'ing the last Skeleton, `battle_wait` returned `[BattleDesertion]` instead of `[BattleVictory]`. The very next `screen` call showed `[BattleVictory] → WorldMap` correctly. Same shape as the post-victory→LoadGame mis-tag (prior playtest #4) — a transient terminal-state race. The existing Victory/GameOver/Desertion flicker tolerance in `BattleAttack` (NavigationActions.cs:1474-1490) might also be needed in the `battle_wait` post-cycle settle. Repro: KO last enemy with `battle_attack`; immediately call `battle_wait` (which polls for screen change). Capture the memory fingerprint at the moment Desertion was reported to find the discriminator.
