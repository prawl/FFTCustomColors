...
- [ ] **🟡 battle_move onto a treasure tile triggers a Yes/No confirm dialog the helper doesn't handle** [Battle] — Discovered 2026-04-25 Siedge Weald. When `battle_move` targets a tile occupied by a fallen unit in `[Treasure]` state (chest), the game pops a "Move to this tile and open the chest? Movement cannot be reset afterwards." Yes/No dialog with Yes pre-selected. The current `battle_move` flow returns at this point assuming the move resolved; caller has to manually confirm with Enter. Two related bugs: (a) detection misclassifies the dialog (showed `[BattleEnemiesTurn]` in live repro — `BattleCrystalMoveConfirm` rule at ScreenDetectionLogic.cs:703 covers crystal/chest move-confirm but didn't fire, signal bytes need re-capture); (b) the move helper should auto-confirm when `pattern` indicates accept (default), or expose an `openChest` flag. Live capture needed: snap detection bytes (encA/B, moveMode, paused, menuCursor, battleMode) at the dialog and add a rule branch.

- [ ] **🟡 Strict mode silently blocks raw Enter key sends** [Bridge] — Discovered 2026-04-25. Only Escape (VK 0x1B) is allow-listed for raw key arrays in strict mode (CommandWatcher.cs:1857). All other raw keys including Enter return `status="blocked"` with the `[STRICT MODE]` error. Today's shell-render fix surfaces the error; remaining gap is no named action exists for "send Enter to a context that doesn't have a named handler" (e.g. arbitrary Yes/No dialogs the bridge doesn't model). Options: (a) add `confirm` named action that wraps Enter; (b) extend strict-mode allowlist to include Enter when on known-prompt screens (BattleChoice / *Confirm); (c) leave as-is and rely on per-screen named actions for every dialog. Recommend (a) — minimal addition, lets the caller dismiss any dialog without modeling each one.
