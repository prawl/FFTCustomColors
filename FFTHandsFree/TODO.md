...
- [~] **ui= field should show "Reset Move" after moving** [Movement] — ResetMove label now renders correctly when cursor is at index 0 post-move. **BUT:** after moving, the game auto-advances the cursor to Abilities (index 1), while `0x1407FC620` still reads 0. So `ui=ResetMove` is shown instead of `ui=Abilities`. Verified 2026-04-12: moved Kenrick, in-game cursor was on Abilities, `ui=` showed ResetMove. Fix: after `battle_move` completes, either re-read the cursor address with a delay, or hardcode that post-move cursor defaults to index 1 (Abilities).
...
- [ ] **Abilities submenu cursor persists within a turn** [Movement] — The Abilities submenu remembers its last cursor position when re-opened. Our `ui=` field always reports "Attack" (index 0) but the actual cursor may be on Jump or Martial Arts from a previous entry. The Up×10 brute-force approach to reset to Attack(0) works IF the submenu is fully loaded, but `ui=` is misleading. Need to either read the actual submenu cursor from memory, or accept that Up×10 resets it and verify via screen detection. Observed 2026-04-12: opened submenu after failed Chakra attempt, `ui=Attack` but cursor was on Jump.
...
- [ ] **Jump is a single command, not 12 separate abilities** [AbilityData] — The in-game Abilities submenu shows a single "Jump" entry, not individual "Horizontal Jump +1/+2/+3..." choices. The learned Jump abilities (Horizontal +1 through +7, Vertical +2 through +8) are passive unlocks that extend the single Jump command's range — the game automatically uses the highest learned level. Our ActionAbilityLookup has 12 separate entries which is wrong. Fix: replace with a single "Jump" entry whose effective HR is determined at scan time from the highest learned Horizontal Jump level in the roster bitfield (bit index in the Jump skillset → maps to range N+1). Similarly, VR = highest learned Vertical Jump level. The calculator then shows one "Jump R:8 VR:8" entry instead of 12.
