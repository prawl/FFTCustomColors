...
Super efficient workflow - test mapping, generate themes, create previews all in one go
"""
...
def create_color_test_sprite():
    """Create a test sprite with distinct colors to map indices"""
    print("\n" + "="*60)
    print("STEP 1: Creating Color Mapping Test")
    print("="*60)
    ...
    # Define test colors for different index ranges
    test_colors = {
        # Accent colors (buckles, trim)
        (3, 6): (255, 215, 0),    # GOLD for indices 3-5

        # Primary armor/cape colors
        (6, 10): (255, 0, 0),     # RED for indices 6-9
        (20, 32): (0, 255, 0),    # GREEN for indices 20-31
        (35, 48): (0, 0, 255),    # BLUE for indices 35-47
        (51, 63): (255, 255, 0),  # YELLOW for indices 51-62

        # Hair indices (10-19) - leave unchanged to preserve
        # Skip index 44 as per documentation
    }

    # Apply test colors to first 8 palettes (unit sprites)
    for palette_idx in range(8):
        base_offset = palette_idx * 32

        for (start_idx, end_idx), color in test_colors.items():
            for idx in range(start_idx, end_idx):
                if idx == 44:  # Skip index 44
                    continue
                if idx >= 10 and idx < 20:  # Skip hair indices
                    continue
    ...
    print(f"✓ Test sprite created: {output_file.name}")
    print("\nColor Mapping Key:")
    print("  GOLD   = Buckles/Trim (indices 3-5)")
    print("  RED    = Primary armor (indices 6-9)")
    print("  GREEN  = Secondary armor (indices 20-31)")
    print("  BLUE   = Extended armor (indices 35-47)")
    print("  YELLOW = Additional armor (indices 51-62)")
...
def generate_theme_colors():
    ...
    # 6. Gradient & Ombre Themes (10)
    for i in range(10):
        hue = i / 10.0
        rgb1 = colorsys.hsv_to_rgb(hue, 0.8, 0.9)
        rgb2 = colorsys.hsv_to_rgb((hue + 0.15) % 1.0, 0.7, 0.7)
        color1 = "#{:02x}{:02x}{:02x}".format(int(rgb1[0]*255), int(rgb1[1]*255), int(rgb1[2]*255))
        color2 = "#{:02x}{:02x}{:02x}".format(int(rgb2[0]*255), int(rgb2[1]*255), int(rgb2[2]*255))
        themes.append((f"gradient_{i+1:02d}", color1, color2))

    # 7. Metallic & Premium (5)
    themes.extend([
...
def create_themed_sprite(source_file, theme_name, primary_color, accent_color):
    ...
    # Define indices to modify (avoiding hair 10-19 and index 44)
    accent_indices = [3, 4, 5]
    primary_indices = list(range(6, 10)) + list(range(20, 32)) + list(range(35, 44)) + list(range(45, 48)) + list(range(51, 63))

    # Apply colors to first 8 palettes
    for palette_idx in range(8):
        base_offset = palette_idx * 32
        ...
        for idx in primary_indices:
            offset = base_offset + (idx * 2)
            if offset + 1 < 512:
                # Special handling for cape edges and shadows
                if idx == 7:
...
def generate_all_themes():
    ...
    print("\n" + "="*60)
    print("STEP 2: Generating 50 Cloud Themes")
    print("="*60)
    ...
    for i, (theme_name, primary, accent) in enumerate(themes, 1):
        print(f"\r[{i:2d}/50] Creating theme: {theme_name:30s}", end="")
...
def convert_all_to_png(themes):
    ...
    print("\n" + "="*60)
    print("STEP 3: Converting All Themes to PNG Previews")
    print("="*60)
    ...
    for i, (theme_name, sprite_file) in enumerate(themes, 1):
        print(f"\r[{i:2d}/50] Converting: {theme_name:30s}", end="")
...
def create_html_gallery(preview_dir, themes):
    ...
    print("\n" + "="*60)
    print("STEP 4: Creating HTML Gallery")
    print("="*60)
...
<body>
    <div class="header">
        <h1>Cloud Strife - 50 Theme Variations</h1>
        <p>Generated preview of all Cloud color themes</p>
    </div>
    ...
    <div class="test-section">
        ...
        <div class="legend">
            ...
            <div class="color-key"><span class="color-box" style="background:#FFD700"></span>GOLD = Buckles/Trim (indices 3-5)</div>
            <div class="color-key"><span class="color-box" style="background:#FF0000"></span>RED = Primary armor (indices 6-9)</div>
            <div class="color-key"><span class="color-box" style="background:#00FF00"></span>GREEN = Secondary armor (indices 20-31)</div>
            <div class="color-key"><span class="color-box" style="background:#0000FF"></span>BLUE = Extended armor (indices 35-47)</div>
            <div class="color-key"><span class="color-box" style="background:#FFFF00"></span>YELLOW = Additional armor (indices 51-62)</div>
        </div>
        <div style="text-align:center">
            <img src="00_COLOR_TEST.png" style="width:128px;height:128px;image-rendering:pixelated;border:3px solid #FFD700">
            <div style="margin-top:10px;color:#FFD700">COLOR MAPPING TEST</div>
        </div>
    </div>
...
"""
    ...
    for i, (theme_name, _) in enumerate(themes, 1):
        html_content += f"""
        <div class="theme">
            <img src="{i:02d}_{theme_name}.png" alt="{theme_name}">
            <div class="theme-name">{theme_name.replace('_', ' ').title()}</div>
        </div>
"""
...
def main():
    ...
    # Step 1: Create color test
    test_sprite = create_color_test_sprite()
    if not test_sprite:
        print("Failed to create test sprite. Exiting.")
        return

    # Step 2: Generate all themes
    ...
    print(f"🖼️ PNG previews: {preview_dir}")
    print(f"🌐 HTML gallery: {gallery_file}")
    print("\nOpening preview directory...")

    # Open the preview directory in Windows Explorer
    if sys.platform == "win32":
        os.startfile(str(preview_dir))
        # Also open the HTML gallery
